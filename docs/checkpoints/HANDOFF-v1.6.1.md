...
8. `.claude/memory/deployment.md` 출시 이력에 v1.6.1 행, `bugs-fixed.md` 70번에 게시 기록
...
```
트랙 확인: `~/fearindex-secrets/play-store-service-account.json`로 androidpublisher v3 `edits.insert` → `tracks.get production` (bugs-fixed 70번 ④ 절차) 또는 `bundle exec fastlane run google_play_track_version_codes track:production` → `[27]`.
...
- 근거·검증: @.claude/memory/bugs-fixed.md 70번(재번호), @.claude/memory/resume-FearIndex-Android.md
