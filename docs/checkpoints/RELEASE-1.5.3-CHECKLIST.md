...
| **알림 내역 표시/영속 정책 분리** | `b196d7f` | 중 | 카피 과약속 + 시계 스큐 데이터 손실 해소. 저장소 무손실로 전환(65번). ⚠️ 저장소 순서 정렬 비보장으로 변경됨 |
| **인터스티셜 세션 리셋 + 재로드(iOS v1.9.2 parity)** | `feature/v1.6.1-interstitial-session-reset` | 중 | KOSPI 진입 전면광고가 프로세스당 1회로 고착되던 결함(68번 ②③) 해소. 30분+ 백그라운드 후 복귀 시 cap/쿨다운/KOSPI 플래그 리셋, 복귀·미준비 시 preload 재시도, cap 도달 시 preload 생략. ⚠️ **1.6.0 소스 push 후 그 위에 재적용해야 함**(현재 dev=1.5.3 기준) |
...
| ~~I~~ | ~~알림내역 prune 시계 의존~~ | — | ~~중~~ | **✅ 해결(65번)** — 표시/영속 분리. 카피 과약속(결제해도 복원 불가)까지 동시 해소. 회귀 7건, 1019 GREEN |
