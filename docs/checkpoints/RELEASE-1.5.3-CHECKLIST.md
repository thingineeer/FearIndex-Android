...
| **인터스티셜 세션 리셋 + 재로드(iOS v1.9.2 parity)** | `feature/v1.6.1-interstitial-session-reset` | 중 | KOSPI 진입 전면광고가 프로세스당 1회로 고착되던 결함(70번 ②③) 해소. 30분+ 백그라운드 후 복귀 시 cap/쿨다운/KOSPI 플래그 리셋, 복귀·미준비 시 preload 재시도, cap 도달 시 preload 생략. ⚠️ **1.6.0 소스 push 후 그 위에 재적용해야 함**(현재 dev=1.5.3 기준) |
