...
/**
 * 포그라운드 복귀 시 인터스티셜 "세션" 리셋 판정 — 순수 로직 (iOS `InterstitialForegroundGate` 구조 동일, 임계값만 다름).
 *
...
object InterstitialForegroundGate {
    ...
    /**
     * 새 세션으로 판정하는 최소 백그라운드 체류 시간 — 10분.
     * iOS 는 GA4 세션 만료 기준 30분이지만 Android 는 사용자 결정(2026-08-23)으로 10분 (ios-parity divergence).
     */
    const val SESSION_RESET_BACKGROUND_MILLIS: Long = 10L * 60L * 1_000L
