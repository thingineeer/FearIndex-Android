...
- **✅ 제보 원인 확정(아버지 Galaxy A35, 화면 크기 기본)**: 증상 = 코스피 진입 **인터스티셜** 안 뜸. 원인 = `InterstitialAdPolicy` 싱글턴 **세션 리셋 호출처 0곳**(KOSPI 진입 광고 프로세스당 1회) + 로드 실패 후 재로드 없음. **iOS v1.9.2(7/13) 가 고친 결함의 Android 미포팅.** **✅ A안(iOS parity) TDD 수정 완료 → dev ba21b53**(30분 백그라운드 후 세션 리셋 + 복귀 preload + 미준비 시 재로드 + cap 도달 시 preload 생략). 유닛 1,037/0, release APK OK. **1.6.0 소스 push 후 1.6.1 로 배포.** 배너 결함(320dp 하한·AndroidView stale)은 별건, 아버지 기기 비해당. 상세: @memory/bugs-fixed.md 68번.
