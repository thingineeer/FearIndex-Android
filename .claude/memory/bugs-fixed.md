...
- **⚠️ RC 상향은 게시·전파 확인 후에만**(23·31번 원칙: 전파 전 올리면 In-App Update 받을 버전이 없어 사용자가 강제창에 갇힘): 준비된 템플릿 = `force_update_minimum_version`[Android]=`1.2`→**`1.6`**, `minimum_app_version`[Android]=`1.2.0`→**`1.6.0`**(diff 2줄, iOS default 1.9.0/1.9.2 불변). 전파 확인 방법: 공개 리스팅 `https://play.google.com/store/apps/details?id=th1ngjin.fearindex&hl=en&gl=US` HTML 에 `"1.6.0"` 등장. 세션 Monitor 가 10분 주기로 감시 중 — 감지 시 deploy(`/private/tmp/.../scratchpad/rc_deploy` 에 firebase.json + rc_template_new.json; 세션 종료 시 `firebase remoteconfig:get -o` 로 다시 받아 2줄만 수정 후 `firebase deploy --only remoteconfig`).
