...
| v1.6.0 | **26** | 2026-08-21 15:35 업로드 | **production 업로드 ✅ (검토 중 → 관리형 게시 OFF, 승인 즉시 게시)** | **App Check 보강**(토큰 선취득+실패 사유 Crashlytics, FCM 등록 정책/재시도) + BoM 33.16.0 + **강제 업데이트 patch 단위 비교**. changelog 26 45 locale "알림 등록 안정성과 보안 검증을 개선하였습니다." 1054 tests GREEN, SHA-1 `CE:08:B4` 일치. **게시·전파 확인 후 RC `force_update_minimum_version`[Android] `1.2`→`1.6`, `minimum_app_version`[Android] `1.2.0`→`1.6.0` 상향 예정(1.5.x 전원 강제)**. 상세: @bugs-fixed.md 69번. 다음 배포 vc27 |
