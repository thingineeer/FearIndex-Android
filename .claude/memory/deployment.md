...
| v1.6.1 | **27** | 2026-08-28 (업로드 12:08 → 공개 리스팅 1.6.1 전파 12:49 KST) | **게시·전파 완료 ✅** (관리형 게시 OFF, 업로드 후 41분 만에 리스팅 반영) | **위젯 전면 리디자인**(1×1 게이지 3종 targetCell 1×1·통합 2×1/2×2 리스트·차트 Global/KOSPI/Crypto 3종 기간 탭·↻ WorkManager·피커 previewImage/label) + 위젯 가이드 Android 재작성 + 앱명 45 locale 현지화 + 온보딩 코치마크 개선 + **등급 원점수 기준·경계 윗 밴드 통일(55.0=탐욕)** + 탐욕 카피 통일 + 인터스티셜 10분 세션 리셋·**KOSPI 진입 즉시 노출(5초 지연 제거)** + ko '줍줍' ASO. changelog 27 45 locale(위젯 반영). 1,132 tests/0, SHA-1 `CE:08:B4` 일치, S22 release ANR 3시나리오 통과. 트랙 production=[27]/internal=[23,22]. ⚠️ 게시·전파 확인 후 RC force `1.6`→`1.6.1` 상향 여부 판단(patch 단위 강제 가능, 69번) |
