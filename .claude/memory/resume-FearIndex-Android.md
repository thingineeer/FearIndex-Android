...
## ⚡ 한 줄 상태
**v1.6.1(vc27) 게시·전파 완료(2026-08-28 업로드 12:08 → 공개 리스팅 1.6.1 노출 12:49 KST, 릴리즈노트 위젯 문구 확인). 다음: RC force 1.6→1.6.1 상향 여부(사용자 결정 대기) → Crashlytics 감시.** 마케팅 급증 대비 점검 GREEN(FCM 등록 99.4%, 신규 123명 임계값 전부 포함), 서버 레이트리밋 60→300/min 전후 대조 종결(71번).
